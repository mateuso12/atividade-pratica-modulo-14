...
public class StatusTarefaController {
	...
	public ResponseEntity<StatusTarefa> findByStatusTarefa(@PathVariable String statusTarefa) {
		StatusTarefa statusTarefa = statusTarefaService.findByStatusTarefa(statusTarefa);
		if(statusTarefa != null)
			return new ResponseEntity<>(statusTarefa, HttpStatus.OK); 
		else 
			return new ResponseEntity<>(null, HttpStatus.NOT_FOUND);		
